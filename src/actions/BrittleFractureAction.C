...
BrittleFractureAction::act()
{
  ...
  else if (_current_task == "add_material")
  {
    ...
    params2.set<std::vector<std::string>>("constant_names") = {"eta"};
    params2.set<std::vector<std::string>>("constant_expressions") = {
        getParam<std::string>("residual_degradation")};
    params2.set<std::string>("function") = "(1-" + _var_name + ")^2*(1-eta)+eta";
    ...
    InputParameters params4 = _factory.getValidParams(type);
    params4.set<std::vector<VariableName>>("args") = {_var_name};
    params4.set<std::string>("f_name") =
        getParam<MaterialPropertyName>("fracture_driving_force_name") + "_" + _var_name;
    params4.set<std::vector<std::string>>("sum_materials") = {
