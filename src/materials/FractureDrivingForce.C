...
validParams<FractureDrivingForce>()
{
  ...
  params.addRequiredParam<std::vector<std::string>>(
      "local_dissipation_names",
      "base names of the material that hold the local dissipation energy density");
  params.addParam<std::string>("driving_force_base_name",
                               "D",
...
FractureDrivingForce::FractureDrivingForce(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _w_names(getParam<std::vector<std::string>>("local_dissipation_names")),
    _D_name(getParam<std::string>("driving_force_base_name") + "_"),
    _E_el_name(getParam<MaterialPropertyName>("elastic_energy_name")),
    _E_el(getMaterialPropertyByName<Real>(_E_el_name)),
    _mobility_base_name(getParam<MaterialPropertyName>("mobility_base_name"))
{
  // reserve space
  ...
  // get mobility
  for (unsigned int i = 0; i < _num_fields; ++i)
    _mobility[i] = &getMaterialPropertyByName<Real>(_mobility_base_name + "_" + _var_names[i]);
  ...
  for (unsigned int i = 0; i < _num_fields; ++i)
  {
    ...
    _w[i] = &getMaterialPropertyByName<Real>(_w_names[i]);
    _dw_dd[i] = &getMaterialPropertyDerivativeByName<Real>(_w_names[i], _var_names[i]);
    _d2w_dd2[i] =
        &getMaterialPropertyDerivativeByName<Real>(_w_names[i], _var_names[i], _var_names[i]);
    // get elastic energy
