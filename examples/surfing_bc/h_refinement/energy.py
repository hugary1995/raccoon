...
with open(sys.argv[1]) as csv_file:
    ...
    for row in csv_reader:
        ...
        elastic_energy.append(float(row[2]))
        fracture_energy.append(
            float(row[2]) + float(row[4]) - initial_fracture_energy)
        plastic_work.append(
            float(row[2]) + float(row[4]) - initial_fracture_energy + float(row[5]))
        external_work.append(float(row[3]))
